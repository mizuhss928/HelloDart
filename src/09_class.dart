main() {
  ...
  engineer1.greed();
  // 親クラスにキャストできる
  Person engineer2 = Person.instance(true);
  // 呼ばれるのは継承クラスのgreed（ポリモフィズム）
  engineer2.greed();
...
class Person {
  ...
  // コンストラクタのブロック内では既に代入された状態で使用できる
  // thisを省略すると別の仮引数として扱われてしまう
  Person(this.firstName, this.lastName);

  // privateメンバは便宜上、_から始まる.
  // ただし、同ライブラリ内であればアクセスしようと思えばアクセスできてしまう
  String? _member;

  // 名前付きコンストラクタ
  // 複数のコンストラクタを持たせたい時に使う
  Person.origin() {
    this.firstName = '氏';
    this.lastName = '名';
  ...
  // 同名のメソッドは作成できない（メソッドのオーバーロード）
  // 引数で処理を分けたい同名のメソッドを作りたければ任意引数を使う
  // greed(int a) {...}
  ...
  factory Person.instance(bool isEngineer) {
...
class Engineer extends Person {
  ...
  // 親クラスのコンストラクタをよぶには初期化子（:）でsuperを使う
  // メンバ変数の初期化も初期化子内で行える
  ...
  // メソッドの上書き
  // @overrideは任意（けど書いた方が分かりやすい）
  @override
  greed() {
    // super.greed(); // メンバ関数内でコンストラクタ以外の親クラスのメンバ関数はsuper.メンバン数名で呼び出しできる
